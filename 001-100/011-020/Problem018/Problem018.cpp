...
* Company: Self
* Description:
*		This is my attempt at problem018 from projecteuler.com
*
...
int main()
{
	...
	for ( int32_t row = layers - 2; row >= 0; row-- )
	{
		for ( uint32_t idx = 0; idx <= row; idx++ )
		{	
			uint32_t s1 = sums.at(idx) + vals.at(row).at(idx);
			uint32_t s2 = sums.at(idx+1) + vals.at(row).at(idx);
			sums.at(idx) = (s1 > s2) ? s1 : s2;
		}
