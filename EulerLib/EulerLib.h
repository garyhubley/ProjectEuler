...
std::vector<std::tuple<uint64_t, uint64_t>> PrimeFactorization(uint64_t n,
...
template <typename tp>
void FwdPrintVector(std::vector<tp> &vec, std::ostream &out = std::cout);
template <typename tp>
void RevPrintVector(std::vector<tp> &vec, std::ostream &out = std::cout);
